...
class STL_Formula(torch.nn.Module):
    ...
    def eval(self, inputs, time=0, pscale=1, scale=-1, **kwargs):
        ...
        """
        return self.eval_trace(inputs, pscale=pscale, scale=scale)[
            :, -(time + 1), :
        ].unsqueeze(
            1
    ...
    def forward(formula, inputs, pscale=1, scale=-1, **kwargs):
        ...
        if isinstance(inputs, Expression):
            assert (
                inputs._value is not None
            ), "Input Expression does not have numerical values"
            return formula.robustness_trace(
                inputs._value, pscale=pscale, scale=scale, **kwargs
            )
        elif isinstance(inputs, torch.Tensor):
            return formula.robustness_trace(
                inputs, pscale=pscale, scale=scale, **kwargs
            )
        elif isinstance(inputs, tuple):
            return formula.robustness_trace(
                convert_to_input_values(inputs), pscale=pscale, scale=scale, **kwargs
            )
        else:
...
class LessThan(STL_Formula):
    ...
    def __init__(self, name="x", val="c"):
        ...
        assert isinstance(
            name, str
        ), "LHS of expression needs to be a string (input name)"
        self.input_name = name
        self.val = val
...
class Until(STL_Formula):
    def __init__(
        self, subformula1="Until subformula1", subformula2="Until subformula2"
    ):
        """
    ...
    def robustness_trace(self, inputs, pscale=1, scale=-1, **kwargs):
        ...
        )  # [batch_size, time_dim, x_dim, time_dim]
        RHS = (
            torch.ones(LHS.shape).to(x.device) * -LARGE_NUMBER
        )  # [batch_size, time_dim, x_dim, time_dim]
        ...
        return maxish(
            minish(torch.stack([LHS, RHS], dim=-1), scale=scale, dim=-1).squeeze(-1),
            scale=scale,
            dim=-1,
        ).squeeze(
            -1
...
class Then(STL_Formula):
    ...
    def robustness_trace(self, inputs, pscale=1, scale=-1, **kwargs):
        ...
        RHS = (
            torch.ones(LHS.shape) * -LARGE_NUMBER
        )  # [batch_size, time_dim, x_dim, time_dim]
        for i in range(trace2.shape[1]):
